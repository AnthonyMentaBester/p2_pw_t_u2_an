...
public class Main {
	...
	public static void main(String[] args) {
		...
		IPersonaPredicate<String> predicatee = MetodosReferenciados::comparar;
		...
		// clases
		highOrder.acceso(consumerHO, "clase consumer");
		// lambdas
		highOrder.acceso(cadena -> {
			LOG.info("cadena con lambda");
		}, "consumer");
		//metodos referenciados
		highOrder.acceso(MetodosReferenciados::aceptar, "consumer");

		//PREDICATE
		IPersonaPredicate<String> predicateHO = new PersonaPredicateImpl();
		// clases
		highOrder.valorar(predicateHO, "clase predicate");
		// lambdas
		highOrder.valorar(cadena -> {
			cadena = "#hola";
			LOG.info("predicate cadena con order");
			if(cadena.contains("#")) {
		...
		//metodos referenciados
		highOrder.valorar(MetodosReferenciados::comparar, "predicate");

		//FUNCTION 
		IPersonaFunction<String, Integer> functionHO = new PersonaFunctionImpl();
		//clases
		highOrder.tranformar(functionHO, 7);
		// lambdas
		highOrder.tranformar(numero -> {
			String valorFinal = numero.toString().concat(" valor con function mediante lambdas ");
			return valorFinal;
		}, 7);
		//metodos referenciados
		highOrder.tranformar(MetodosReferenciados::aplicar, 8);
		
		//UNARY OPERATOR
		IPersonaUnaryOperator<Integer> unaryOperatorHO = new PersonaUnaryOperatorImpl();
		//clases
		highOrder.mantener(unaryOperatorHO, 6);
		// lambdas
